...
class OpticalFlow:
    def __init__(self, is_compensation=False, train=True, trainable=True, reuse=None, regularizer=None, is_scale=True, is_bidirection=False, name='flow'):
        self.train = train
    ...
    def __call__(self, input, features):
        with tf.variable_scope(self.name):
            ...
            com1_features, com2_features = features
            diff_cost_fw = dict()
            diff_cost_bw = dict()
            if self.is_bidirection == False:
                ...
                flow_estimated = layer._pyramid_processing(x1_feature, x2_feature, img_size, compensate_vol=None, is_compensation=False, train=self.train, trainable=self.trainable, regularizer=self.regularizer, is_scale=self.is_scale, name='pyramid_processing') 
                ...
                if self.is_compensation == True:
                    # 执行diff cost fw操作
                    diff_cost_fw = layer.diff_cost_extractor(com1_features, com2_features, x1_feature, x2_feature, reuse=self.reuse)
                flow_fw = layer._pyramid_processing(x1_feature, x2_feature, img_size, compensate_vol=diff_cost_fw, is_compensation=self.is_compensation, train=self.train, trainable=self.trainable, reuse=self.reuse, regularizer=self.regularizer, is_scale=self.is_scale, name='pyramid_processing')
                
                if self.is_compensation == True:
                    # 执行diff cost bw操作
                    diff_cost_bw = layer.diff_cost_extractor(com2_features, com1_features, x2_feature, x1_feature, reuse=True)
                flow_bw = layer._pyramid_processing(x2_feature, x1_feature, img_size, compensate_vol=diff_cost_bw, is_compensation=self.is_compensation, train=self.train, trainable=self.trainable, reuse=True, regularizer=self.regularizer, is_scale=self.is_scale, name='pyramid_processing') 
                self.variables = tf.get_collection(tf.GraphKeys.TRAINABLE_VARIABLES, scope=self.name)
                self.reuse = True

                if self.is_compensation == True:
                    return x1_feature, x2_feature, flow_fw, flow_bw, diff_cost_fw
                else:
                    return flow_fw, flow_bw
                
                # return x1_feature, x2_feature, flow_fw, flow_bw
