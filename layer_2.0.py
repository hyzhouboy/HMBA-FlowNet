...
def conv_block(x, channel, reuse=None, trainable=True, regularizer=None, name='feature_conv'):
    with tf.variable_scope(name, reuse=reuse, regularizer=regularizer):
        with slim.arg_scope([slim.conv2d], activation_fn=None, kernel_size=3, padding='SAME', trainable=trainable):
            net = {}
            net['block1_1'] = slim.conv2d(x, channel, stride=1, scope='conv1_1')
            net['block1_2'] = slim.conv2d(net['block1_1'], channel, stride=1, scope='conv1_2')
            net['residual_block1_relu'] = lrelu(net['block1_2'])

            net['block2_1'] = slim.conv2d(net['residual_block1_relu'], channel, stride=1, scope='conv2_1')
            net['block2_2'] = slim.conv2d(net['block2_1'], channel, stride=1, scope='conv2_2')
            net['residual_block2_relu'] = lrelu(net['block2_2'])

            net['block3_1'] = slim.conv2d(net['residual_block1_relu'], channel, stride=1, scope='conv3_1')
            net['block3_2'] = slim.conv2d(net['block3_1'], channel, stride=1, scope='conv3_2')                
            net['residual_block3_relu'] = lrelu(net['block3_2'])
    out_cost = net['residual_block3_relu']
    return out_cost

def estimator(x1, x2, flow, compensate_vol=None, is_compensation=False, train=True, trainable=True, reuse=None, regularizer=None, name='estimator'):
    ...
    if is_compensation == True:
      cost_volume += compensate_vol
...
def _pyramid_processing(x1_feature, x2_feature, img_size, compensate_vol=None, is_compensation=False, train=True, trainable=True, reuse=None, regularizer=None, is_scale=True, name='pyramid_processing'):
    with tf.variable_scope(name, reuse=reuse, regularizer=regularizer):
        x_shape = tf.shape(x1_feature['conv6_2'])

        initial_flow = tf.zeros([x_shape[0], x_shape[1], x_shape[2], 2], dtype=tf.float32, name='initial_flow')
        flow_estimated = {}
        flow_estimated['level_6'] = estimator(x1_feature['conv6_2'], x2_feature['conv6_2'], initial_flow, compensate_vol['conv6_2'],
         is_compensation=is_compensation, train=train, trainable=trainable, reuse=reuse, regularizer=regularizer, name='estimator_level_6')['conv6']
        ...
        for i in range(4):
            ...
            initial_flow = flow_resize(flow_estimated['level_%d' % (6-i)], feature_size, is_scale=is_scale)
            if i == 3:
                estimator_net_level_2 = estimator(x1_feature[feature_name], x2_feature[feature_name], initial_flow, compensate_vol[feature_name],
                    is_compensation=is_compensation, train=train, trainable=trainable, reuse=reuse, regularizer=regularizer, name='estimator_level_%d' % (5-i))
                flow_estimated['level_2'] = estimator_net_level_2['conv6']
            else:
                flow_estimated['level_%d' % (5-i)] = estimator(x1_feature[feature_name], x2_feature[feature_name], initial_flow, compensate_vol[feature_name],
                    is_compensation=is_compensation, train=train, trainable=trainable, reuse=reuse, regularizer=regularizer, name='estimator_level_%d' % (5-i))['conv6']
...
def diff_cost_extractor(x1_features, x2_features, y1_features, y2_features, level=6, reuse=False):
  diff_cost = {}
  d = 9
  for i in range(level):
    feature_name = 'conv%d_2' % (6-i)
    diff_feature1 = x1_features[feature_name] - y1_features[feature_name]
    diff_feature2 = x2_features[feature_name] - y2_features[feature_name]
    features = tf.concat([diff_feature1, diff_feature2], axis=-1)
    diff_cost_volume = conv_block(features, channel=d*d, reuse=reuse, trainable=True, regularizer=slim.l2_regularizer(scale=1e-4), name='diff_cost'+str(i))
    diff_cost[feature_name] = diff_cost_volume
    reuse=True
